 ...
 * the read-only font diagnostics surface `SuperDocFontsApi`. Drains the
 ...
 * `FontResolutionRecord[]`; `onReport` streams `FontsChangedPayload` and
 * returns an unsubscribe. Every member resolves to a named public type, so a
 * consumer gets real IntelliSense with no `any` at depth.
 *
 * Drained obligation (1):
 *   - fonts:returns
 */
import type { FontResolutionRecord, FontsChangedPayload, SuperDoc, SuperDocFontsApi } from 'superdoc';
...
void [_fontsOk, _missing, _documentFonts, _unsubscribe];
