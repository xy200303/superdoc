...
const MCP_GENERATED_DIR = path.join(REPO_ROOT, 'apps/mcp/src/generated');
...
export async function generateIntentTools(contract) {
  ...
  const writes = [
    ...
    writeGeneratedFile(
      path.join(MCP_GENERATED_DIR, 'catalog.ts'),
    ...
    // MCP is a self-contained bundle and does not keep @superdoc-dev/sdk as a runtime dependency.
    // Emit a local copy of dispatch so the generated catalog and dispatch stay pinned together.
    writeGeneratedFile(
      path.join(MCP_GENERATED_DIR, 'intent-dispatch.generated.ts'),
      dispatchTs.replace(
        '// Auto-generated by generate-intent-tools.mjs — do not edit',
  ...
  try {
    ...
    // Write assembled MCP prompt for SDK consumers that call getMcpPrompt().
    ...
    // MCP server: embed MCP prompt into the bundle to avoid runtime asset path relocation.
    const mcpEscaped = mcpPrompt.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$/g, '\\$');
    const mcpPromptTs =
      '// Auto-generated from tools/prompt-templates/system-prompt-mcp-header.md + system-prompt-core.md\n' +
      '// Do not edit manually — re-run generate:all to update.\n' +
      'export const MCP_SYSTEM_PROMPT = `' + mcpEscaped + '`;\n';
    writes.push(writeGeneratedFile(path.join(MCP_GENERATED_DIR, 'mcp-prompt.ts'), mcpPromptTs));
  } catch {
