...
describe('table live xml test', () => {
  ...
  it('parses simple xml', () => {
    ...
    const runR1C1 = result.nodes[0].content[0].content[0].content[0].content[0];
    expect(runR1C1.type).toBe('run');
    const textR1C1 = runR1C1.content.find((child) => child.type === 'text');
    expect(textR1C1?.text).toBe('COL 1 ROW 1');
    expect(extractParagraphText(result.nodes[0].content[0].content[0].content[0])).toBe('COL 1 ROW 1');
    expect(result.nodes[0].content[0].content[1].content[0].type).toBe('paragraph');
    const runR1C2 = result.nodes[0].content[0].content[1].content[0].content[0];
    expect(runR1C2.type).toBe('run');
    const textR1C2 = runR1C2.content.find((child) => child.type === 'text');
    expect(textR1C2?.text).toBe('COL 2 ROW 1');
    expect(extractParagraphText(result.nodes[0].content[0].content[1].content[0])).toBe('COL 2 ROW 1');
    ...
    const runR2C1 = result.nodes[0].content[1].content[0].content[0].content[0];
    expect(runR2C1.type).toBe('run');
    const textR2C1 = runR2C1.content.find((child) => child.type === 'text');
    expect(textR2C1?.text).toBe('COL 1 ROW 2');
    expect(extractParagraphText(result.nodes[0].content[1].content[0].content[0])).toBe('COL 1 ROW 2');
    expect(result.nodes[0].content[1].content[1].content[0].type).toBe('paragraph');
    const runR2C2 = result.nodes[0].content[1].content[1].content[0].content[0];
    expect(runR2C2.type).toBe('run');
    const textR2C2 = runR2C2.content.find((child) => child.type === 'text');
    expect(textR2C2?.text).toBe('COL 2 ROW 2');
...
describe('table tests to check colwidth', () => {
  ...
  it('aligns column widths when rows define grid offsets', async () => {
    const docx = await getTestDataByFileName('table-width-issue.docx');
    const documentXml = docx['word/document.xml'];
    ...
    table.content.forEach((row) => {
      row.content.forEach((cell) => {
        ...
        const widths = Array.isArray(cell.attrs.colwidth) ? cell.attrs.colwidth : [];
        expect(widths.length).toBe(span);
      });
    });
    const gridTwips = table.attrs.grid.map((item) => item.col);
    expect(gridTwips).toEqual([8, 3413, 561, 1714, 1130, 1145, 1707, 1698, 8]);

    const gridWidthsPx = gridTwips.map((value) => Number(twipsToPixels(value).toFixed(3)));

    const columnCount = gridWidthsPx.length;
    const activeRowSpans = new Array(columnCount).fill(0);

    table.content.forEach((row) => {
      const rowProps = row.attrs.tableRowProperties || {};
      const gridBefore = rowProps.gridBefore || 0;
      const gridAfter = rowProps.gridAfter || 0;

      let columnCursor = 0;
      let placeholderColumns = 0;

      const advancePastSpans = () => {
        while (columnCursor < columnCount && activeRowSpans[columnCursor] > 0) {
          activeRowSpans[columnCursor] -= 1;
          columnCursor += 1;
        }
      };
      ...
      row.content.forEach((cell) => {
        ...
        const expectedSlice = gridWidthsPx.slice(columnCursor, columnCursor + colspan);
        const actualWidths = (cell.attrs.colwidth || []).map((value) => Number(Number(value).toFixed(3)));

        expect(actualWidths).toHaveLength(expectedSlice.length);

        if (cell.attrs.__placeholder) {
          placeholderColumns += colspan;
          expectedSlice.forEach((expectedWidth, index) => {
            const diff = Math.abs(actualWidths[index] - expectedWidth);
        ...
        if (rowspan > 1) {
          for (let offset = 0; offset < colspan; offset += 1) {
            const index = columnCursor + offset;
            expect(index).toBeLessThan(columnCount);
            activeRowSpans[index] = Math.max(activeRowSpans[index], rowspan - 1);
          }
        }
      ...
      expect(placeholderColumns).toBe(gridBefore + gridAfter);
      expect(columnCursor).toBe(columnCount);
      if (gridBefore > 0) {
        expect(row.content[0].attrs.__placeholder).toBeDefined();
      }
    });
  });
});
