...
import { translator } from './adjustRightInd-translator.js';

describe('w:adjustRightInd translator', () => {
  describe('encode', () => {
  ...
  describe('decode', () => {
    it('creates a w:adjustRightInd element if adjustRightInd is true', () => {
      const { attributes: result } = translator.decode({ node: { attrs: { adjustRightInd: true } } });
  ...
  it('has correct metadata', () => {
    expect(translator.xmlName).toBe('w:adjustRightInd');
    expect(translator.sdNodeOrKeyName).toBe('adjustRightInd');
  });
});
