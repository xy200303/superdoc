...
import { translator } from './snapToGrid-translator.js';

describe('w:snapToGrid translator', () => {
  describe('encode', () => {
  ...
  describe('decode', () => {
    it('creates a w:snapToGrid element if snapToGrid is true', () => {
      const { attributes: result } = translator.decode({ node: { attrs: { snapToGrid: true } } });
  ...
  it('has correct metadata', () => {
    expect(translator.xmlName).toBe('w:snapToGrid');
    expect(translator.sdNodeOrKeyName).toBe('snapToGrid');
  });
});
