...
import { translator } from './gridAfter-translator.js';

describe('w:gridAfter translator', () => {
  describe('encode', () => {
  ...
  describe('decode', () => {
    ...
    it('returns undefined if gridAfter property is missing', () => {
      expect(translator.decode({ node: { attrs: {} } })).toBeUndefined();
    });

    it('returns undefined if gridAfter property is not a number', () => {
      expect(translator.decode({ node: { attrs: { gridAfter: null } } })).toBeUndefined();
      expect(translator.decode({ node: { attrs: { gridAfter: 'hello' } } })).toBeUndefined();
    });
  });

  it('has correct metadata', () => {
    expect(translator.xmlName).toBe('w:gridAfter');
    expect(translator.sdNodeOrKeyName).toBe('gridAfter');
  });
});
