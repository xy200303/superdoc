...
import { translator } from './tblInd-translator.js';

describe('w:tblInd translator', () => {
  describe('encode', () => {
  ...
  describe('decode', () => {
    it('creates a w:tblInd element with w:w and w:type attributes', () => {
      const { attributes: result } = translator.decode({
        node: { attrs: { tableIndent: { value: 140, type: 'pct' } } },
      });
      expect(result).toEqual({ 'w:w': '140', 'w:type': 'pct' });
    });

    it('handles missing type property', () => {
      const { attributes: result } = translator.decode({ node: { attrs: { tableIndent: { value: 140 } } } });
      expect(result).toEqual({ 'w:w': '140' });
    });

    it('returns undefined if tableIndent property is missing', () => {
      expect(translator.decode({ node: { attrs: {} } })).toBeUndefined();
    });

    it('returns undefined if tableIndent.value is missing or not a number', () => {
      expect(translator.decode({ node: { attrs: { tableIndent: { type: 'dxa' } } } })).toBeUndefined();
      expect(translator.decode({ node: { attrs: { tableIndent: { value: null } } } })).toBeUndefined();
    });
  });

  it('has correct metadata', () => {
    expect(translator.xmlName).toBe('w:tblInd');
    expect(translator.sdNodeOrKeyName).toBe('tableIndent');
  });
});
