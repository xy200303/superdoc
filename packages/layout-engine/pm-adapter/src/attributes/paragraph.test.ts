...
describe('computeParagraphAttrs', () => {
  ...
  // SD-3269 follow-up: Word ties the marker's vanish to the FIRST inline run.
  // If the first run carries w:vanish the marker is hidden together with the
  // leading content; pPr/rPr vanish on its own does not hide the marker.
  it('hides the list marker when the first inline run is vanished', () => {
    const paragraph: PMNode = {
      type: { name: 'paragraph' },
      attrs: {
        paragraphProperties: {
      ...
      content: [
        {
          type: 'run',
          attrs: { runProperties: { vanish: true } },
          content: [{ type: 'text', text: 'Hidden heading seed ' }],
        },
        {
          type: 'run',
          attrs: { runProperties: { underline: { 'w:val': 'single' } } },
          content: [{ type: 'text', text: 'Assets Purchased' }],
        },
      ],
    };
    ...
    const minimalContext = {
      translatedNumbering: {
        definitions: { '1': { numId: 1, abstractNumId: 1 } },
        abstracts: {
          '1': { abstractNumId: 1, levels: { '0': { ilvl: 0, runProperties: {} } } },
        },
      },
      translatedLinkedStyles: { docDefaults: {}, styles: {} },
      tableInfo: null,
  ...
  it('keeps the list marker visible when the first inline run is not vanished', () => {
    const paragraph: PMNode = {
      type: { name: 'paragraph' },
      attrs: {
        paragraphProperties: {
