...
<Tip>
  Want full control over the toolbar UI? On React, use [Bring Your Own UI](/bring-your-own-ui/toolbar-and-commands) — typed hooks (`useSuperDocCommand`), per-command re-renders, custom commands. For non-React stacks, use the [headless toolbar](/modules/toolbar/headless).
</Tip>
