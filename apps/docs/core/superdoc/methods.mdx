...
<Note>
**`addCommentsList` and `removeCommentsList` mount the built-in comments sidebar.** For custom React UI, render your own panel from `useSuperDocComments` instead — see [Bring Your Own UI › Comments](/bring-your-own-ui/comments). The methods below stay supported for apps using the built-in sidebar.
</Note>
